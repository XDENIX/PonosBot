 const Command = require("../BotCommand");
    ...
    for(const file of findFiles(handlerPath)){
        try {
            ...
            fn(props, base, path.parse(dir).name)
             console.log(`[ COMMANDS ] Command ${file} successfully loaded !`)
        }catch(err) {
            console.error(`[ COMMANDS ] Command ${file} failed to load. ${err}`)
        }
    }
}

module.exports = async(client) => folderHandler('./src/commands/', (props, file, folder) => { props.module = folder; client.stored.commands.set(props.name, new Command(props)) });
